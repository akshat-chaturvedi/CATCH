    ...
    if args.find:
        if args.gaia:
            if args.instrument == "HK":
                hk_cal_finder(args.science_target, gaia_comp_check=5)
            elif args.instrument == "S":
                s_cal_finder(args.science_target, gaia_comp_check=5)
            elif args.instrument == "R":
                r_cal_finder(args.science_target, gaia_comp_check=5)

        else:
            if args.instrument == "HK":
                hk_cal_finder(args.science_target)
            elif args.instrument == "S":
                s_cal_finder(args.science_target)
            elif args.instrument == "R":
                r_cal_finder(args.science_target)
...
def main():
    # Check CDS Vizier mirror is up, if not, switch to Harvard/CfA mirror
    print(f"Vizier server: {GREEN}{conf.server}{RESET}")
    try:
        vizier_web_status_check = requests.get("https://" + f"{conf.server}", timeout=1)
        vizier_web_status = vizier_web_status_check.status_code

    except requests.exceptions.Timeout:
        print(f"{YELLOW}WARNING: Default Vizier server timed out! Switching servers{RESET}")
    ...
    if vizier_web_status != 200:
        print(
            ...
            f"switching to {GREEN}vizier.cfa.harvard.edu{RESET}"
        )
    ...
    # If no system arguments provided, run in interactive mode
    if len(sys.argv) <= 1:
        ...
        interactive_questions(instrument_question)

    # If sys args provided, run in CLI mode
    else:
        cli_questions()
...
if __name__ == '__main__':
    print(
        f"""
        ################################ {RED}C{RESET}{GREEN}A{RESET}{YELLOW}T{RESET}{BLUE}C{RESET}{MAGENTA}H{RESET} ################################
