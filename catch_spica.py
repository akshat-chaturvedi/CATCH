...
def R_cal_finder(star_name: str, gaia_comp_check: int | float | None = None) -> None:
    ...
    # Also checking with GAIA (G_Rp<=5.4)
    ...
    # rotation checks
    vizier = Vizier(columns=["_RAJ2000", "_DEJ2000", "IPDfmp", "RUWE", "RVamp", "Vbroad", "+_r", "RPmag"],
    ...
    if len(gaia_result) > 0:
        ...
        exit("ERROR: No calibrators found within 20 degrees of your target in Gaia DR3. Consider modifying your "
             "constraints!")
    if gaia_comp_check:
        vizier_neighbors = Vizier(columns=["_RAJ2000", "_DEJ2000", "IPDfmp", "RUWE", "RVamp", "Vbroad", "+_r", "RPmag"],
                                  catalog="I/355/gaiadr3")
    ...
    if len(jmmc_result) != 0:
        ...
        if (jmmc_result['Vmag'] <= 5) |  (jmmc_result['Hmag'] <= 6.4) | (jmmc_result['Rmag'] <= 5.4) | (jmmc_result['UDDH'] > 0.25):
            ...
            if jmmc_result['Vmag'] > 5:
                print(f"---->{RED}Calibrator Vmag > 5! May be too faint for reliable AO acquisition!{RESET}")
